...
const submitSchema = z.object({
  ...
  advancePurpose: z.string().max(120).optional(),
  advancePurposeDetail: z.string().max(300).optional(),
  ...
  paymentFrequency: z.enum(["WEEKLY", "DAILY"]).optional(),
...
export async function submitApplication(input: z.infer<typeof submitSchema>) {
  ...
  const application = await prisma.application.create({
    data: {
      ...
      advancePurpose: data.advancePurpose || null,
      advancePurposeDetail: data.advancePurposeDetail || null,
...
export async function fundApplication(applicationId: string, fundedAmount: number) {
  ...
  // status optimistically, which silently hid errors and left the borrower
  // thinking they were funded.
  ...
  await prisma.application.update({
    ...
    data: {
      ...
      ...(goachDisburseUuid ? { goachDisburseUuid } : {}),
    },
  });
