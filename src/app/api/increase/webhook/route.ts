...
async function refreshApplicationStatusFromPayments(applicationId: string): Promise<void> {
  ...
  // Only still-PENDING (not yet collected) payments count as overdue — a
  // PROCESSING payment is in-flight at the processor (GoACH settles in ~9
  // days), so it isn't "late". Anchor "today" to Eastern Time so an advance
  // due Jun 8 doesn't flip to LATE at 8 PM EDT (when UTC rolls to Jun 9).
  ...
  // A rolled-away miss or an in-flight (PROCESSING) debit counts as activity
  // too — only a truly untouched advance stays FUNDED.
  ...
  else if (hasPaidPayment || hasRolled || hasProcessing) nextStatus = "REPAYING";
