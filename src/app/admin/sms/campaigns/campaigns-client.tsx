...
export function SmsCampaignsClient({ campaigns }: { campaigns: Campaign[] }) {
  const router = useRouter();
  const [search, setSearch] = useState("");
  const [statusFilter, setStatusFilter] = useState("all");

  const filtered = campaigns
    .filter((c) => c.name.toLowerCase().includes(search.toLowerCase()) || c.body.toLowerCase().includes(search.toLowerCase()))
  ...
  return (
    <div>
      <PageHeader
        title="SMS Campaigns"
        description="One-time SMS sends to contact segments"
        action={{ label: "+ New Campaign", href: "/admin/sms/campaigns/new" }}
      />
      ...
      <div className="bg-white rounded-xl border border-[#e4e4e7] overflow-hidden">
        {filtered.length === 0 ? (
          <div className="text-center py-16 text-[#a1a1aa]">
            <p className="text-[15px] font-medium mb-1">No SMS campaigns yet</p>
            <p className="text-[13px]">Create your first one to start sending texts.</p>
          </div>
        ) : (
          <table className="w-full">
            <thead>
              <tr className="border-b border-[#e4e4e7]">
                ...
                <th className="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-[0.06em] text-[#a1a1aa]">Delivered</th>
                <th className="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-[0.06em] text-[#a1a1aa]">Failed</th>
            ...
            <tbody>
              {filtered.map((c) => (
                <tr
                  ...
                  onClick={() => router.push(`/admin/sms/campaigns/${c.id}`)}
                >
                  <td className="px-4 py-3">
                    <p className="text-[13px] font-semibold text-black">{c.name}</p>
                    <p className="text-[12px] text-[#71717a] truncate max-w-[280px]">{c.body}</p>
                  </td>
                  <td className="px-4 py-3"><StatusBadge status={c.status} /></td>
                  <td className="px-4 py-3 text-[13px] text-black">{c.totalSent.toLocaleString()}</td>
                  <td className="px-4 py-3 text-[13px] text-black">{c.totalDelivered.toLocaleString()}</td>
                  <td className="px-4 py-3 text-[13px] text-black">{c.totalFailed.toLocaleString()}</td>
      ...
      <div className="mt-4">
        <Link href="/admin/sms/campaigns/new" className="text-[13px] font-medium text-[#15803d] hover:underline">
          + New Campaign
        </Link>
      </div>
    </div>
