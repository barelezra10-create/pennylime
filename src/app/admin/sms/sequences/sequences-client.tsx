...
import { updateSmsSequence } from "@/actions/sms";
...
export function SmsSequencesClient({ sequences }: { sequences: Sequence[] }) {
  ...
  async function toggleActive(id: string, current: boolean) {
    try {
      await updateSmsSequence(id, { active: !current });
  ...
  return (
    <div>
      <PageHeader
        title="SMS Sequences"
        description="Automated SMS drip sequences triggered by contact events"
        action={{ label: "+ New Sequence", href: "/admin/sms/sequences/new" }}
      />
      ...
      <div className="bg-white rounded-xl border border-[#e4e4e7] overflow-hidden">
        {filtered.length === 0 ? (
          <div className="text-center py-16 text-[#a1a1aa]">
            <p className="text-[15px] font-medium mb-1">No SMS sequences yet</p>
            ...
            <tbody>
              {filtered.map((s) => (
                <tr
                  ...
                  onClick={() => router.push(`/admin/sms/sequences/${s.id}`)}
                >
